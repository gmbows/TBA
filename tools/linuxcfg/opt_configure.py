...
makestr += execname+": "+" ".join(binlist)+"\n\tg++ -O3 -std=c++11 "+" ".join(binlist)+" `sdl2-config --cflags --libs` -lSDL2main -lSDL2_image -o "+execname+"\n\n"

for src in srcs:
	head=""
	if src in heads:
		head = src+".h"
	makestr += "bin/{1}.o: {0}.cpp {2} \n\tg++ -O3 -std=c++11 -c {0}.cpp `sdl2-config --cflags --libs` -lSDL2main -lSDL2_image -o bin/{1}.o\n\n".format(src,src.replace(gsource_dir,"").replace(uisource_dir,"").replace(main_dir,""),head)
