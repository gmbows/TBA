...
makestr += execname+": "+" ".join(binlist)+"\n\tg++ -std=c++11 "+" ".join(binlist)+" `sdl2-config --libs --cflags` -lSDL2main -lSDL2_image -o "+execname+"\n\n"

for src in srcs:
	head=""
	if src in heads:
		head = src+".h"
	makestr += "bin/{1}.o: {0}.cpp {2} \n\tg++ -std=c++11 -c {0}.cpp `sdl2-config --libs --cflags` -lSDL2main -lSDL2_image -o bin/{1}.o\n\n".format(src,src[src.rfind('/')+1:],head)
