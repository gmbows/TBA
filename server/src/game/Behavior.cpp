...
void Character::setLocomotion() {
	...
	if(this->hasTarget()) {
		if(this->hasStatus(STATUS_PURSUE)) {
			...
			if(this->hasTarget()) this->moveAway(this->getCharTarget());
		}
	}
...
void Character::receiveAttack(int damage,GameObject *attacker) {
	...
	if(this->isPlayer()) {
		...
		// TBAGame->displayText(this->getName()+" eceived "+std::to_string(damage)+" damage from "+attacker->getName());
	} else {
	...
	if(static_cast<Character*>(attacker)->isPlayer() or true) {
		TBAGame->displayText(attacker->getEntityName()+" deals "+std::to_string(damage)+" damage to "+this->getEntityName());
	}
	...
	if(this->health <= 0) {
		TBAGame->displayText(attacker->getEntityName()+" kills "+this->getEntityName());
...
void Character::say(const std::string& message) {
	TBAGame->displayText(this->getName()+" says \""+message+"\"");
